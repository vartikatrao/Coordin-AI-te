...
#         intent_task = Task(
#                 description=f"""
#                 Use GroupIntentExtractorTool with:
#                 members_data={json.dumps(members)},
#                 fair_coords={json.dumps(fair_coords)},
...
# def create_group_agent():
#     return GroupCoordinationAgent()
#the above one was right. check once 
...
from app.agents.tools.group_intent_extractor_tool import GroupIntentExtractorTool
from app.agents.tools.foursquare_tool_group import FoursquareGroupTool
from app.agents.tools.safety_tools import SafetyAssessmentTool, SafetyExplainerAgent
from app.agents.tools.location_resolver import resolve_location, compute_fair_coordinates
...
class GroupCoordinationAgent:
    ...
    def setup_tools(self):
        ...
        self.venue_tool = FoursquareGroupTool()
        self.safety_tool = SafetyAssessmentTool()
        self.safety_explainer = SafetyExplainerAgent()
    ...
    def setup_agents(self):
        ...
        self.safety_agent = Agent(
            role="Safety Assessor",
            goal="Assess safety of the area and venues",
            backstory="Analyzes hospitals, police stations, open venues, and time-of-day",
            verbose=True,
            allow_delegation=False,
            llm=llm,
            tools=[self.safety_tool]
        ...
        self.safety_explainer_agent = Agent(
            role="Safety Explainer",
            goal="Explain safety context and compute safety score if possible",
            backstory="Uses LLM reasoning on hospitals/police/venues data to give a human-friendly safety explanation.",
            verbose=True,
            allow_delegation=False,
            llm=llm
        )
    ...
    def create_tasks(self, members: List[Dict[str, str]], fair_coords: Dict[str, float], meeting_time: Optional[str] = None):
        ...
        safety_task = Task(
            description=f"Assess safety of fair coords {fair_coords} and the venues at meeting time {meeting_time or current_time}",
            agent=self.safety_agent,
            expected_output="Safety JSON with hospitals, police, open venues, context",
            context=[venue_task]
        )

        safety_explainer_task = Task(
            description=f"""
            Take the JSON from SafetyAssessmentTool and explain the results. 
            If at least 1 hospital, 1 police, and 5 open venues are found, compute a safety_score (0–100). 
            Otherwise, only explain in natural language.
            """,
            agent=self.safety_explainer_agent,
            expected_output="JSON with 'explanation' and optional 'safety_score'",
            context=[safety_task]
        )
        ...
        personalization_task = Task(
            ...
            agent=self.personalizer_agent,
            expected_output="JSON mapping each venue to member_name: reason",
            context=[intent_task, venue_task, safety_task, safety_explainer_task]
        )

        return [intent_task, venue_task, safety_task, safety_explainer_task, personalization_task]
    ...
    async def coordinate_group_meetup(self, members: List[Dict[str, str]], meeting_time: Optional[str] = None) -> Dict[str, Any]:
        ...
        crew = Crew(
            agents=[self.intent_agent, self.venue_agent, self.safety_agent, self.safety_explainer_agent, self.personalizer_agent],
            tasks=tasks,
            verbose=True,
            process=Process.sequential
        )
