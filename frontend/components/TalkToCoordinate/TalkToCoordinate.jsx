...
const TalkToCoordinate = () => {
  ...
  const generateConversationTitle = async (message) => {
    try {
      const response = await fetch('https://vartikatrao.me/api/v1/solo/generate-title', {
        method: 'POST',
        headers: {
  ...
  const handleExampleClick = async (exampleText) => {
    ...
    try {
      ...
      // Add AI message to local state immediately for instant UI response
      ...
      try {
        ...
        console.log('AI message saved to Firebase successfully');
      } catch (firebaseError) {
        console.error('Error saving AI message to Firebase:', firebaseError);
        // Keep the message in local state even if Firebase fails
      }
  ...
  const generateAIResponse = async (userQuery) => {
    try {
      ...
      const response = await fetch('https://vartikatrao.me/api/v1/solo/query', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (result.status === 'success') {
        // Extract text response from backend and return as markdown content
        let responseText = "";
        ...
        if (result.data) {
          ...
          if (result.data.recommendations) {
            if (typeof result.data.recommendations === 'string') {
              responseText = result.data.recommendations;
            } else if (result.data.recommendations.raw) {
              responseText = result.data.recommendations.raw;
              ...
              if (recommendationTask?.raw) {
                responseText = recommendationTask.raw;
              }
            }
        ...
        // If we still don't have content, try to convert result to string
        ...
        // Return simplified structure that just renders as markdown
        return {
          markdownContent: responseText || "I generated some recommendations for you, but had trouble formatting them. Please try asking again."
        };
      } else {
      ...
      if (query.includes('coffee') || query.includes('cafe')) {
        return {
          markdownContent: `# Coffee Recommendations

I'm having trouble connecting to my AI services, but here are some general coffee suggestions:
...
*For more specific recommendations, please try again later when my AI services are available.*`
        };
      } else if (query.includes('food') || query.includes('restaurant') || query.includes('eat')) {
        return {
          markdownContent: `# Dining Recommendations

I'm having trouble connecting to my AI services, but here are some general dining tips:

- 🍕 Search for restaurants by cuisine type
- 🍜 Check ratings and reviews
- 🍔 Consider location and parking options

*For personalized recommendations, please try again later when my AI services are available.*`
        };
      } else {
        return {
...
**Please try:**
- Ask about specific types of places (coffee, food, study, etc.)
- Be specific about your preferences and location
- Try again in a few moments

*For the best experience, please try again later when my AI services are fully available.*`
        };
      }
    }
  };

  // Note: Simplified to render backend response directly as markdown
                ...
                {message.content.markdownContent ? (
                  <ReactMarkdown
                    components={{
                      ...
                      ol: ({children}) => <Box ml={4} mb={3}>{children}</Box>,
                      ...
                      hr: () => <Divider my={4} />,
                      blockquote: ({children}) => (
                        <Box bg="blue.50" p={3} borderRadius="md" borderLeft="4px solid" borderColor="blue.400" my={3}>
                          {children}
                        </Box>
                      ),
                      code: ({children}) => (
                        <Text as="code" bg="gray.100" px={1} py={0.5} borderRadius="sm" fontSize="sm" fontFamily="mono">
                          {children}
                        </Text>
                      ),
                    }}
                  ...
                  <Text fontSize="sm" color="gray.600">
                    I generated a response, but had trouble formatting it. Please try asking again.
                  </Text>
                )}
              </Box>
            )}
